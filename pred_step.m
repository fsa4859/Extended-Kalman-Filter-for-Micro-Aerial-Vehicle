...
Q=0.05*dt*diag(process_variances);
...
if isempty(counter)
    ...
    %xdot=[vx-nvx;vy-nvy;vz-nvz;gInvMatrix(1,:)*(angVel(:,1)-bg(:,1)-ng(:,1));gInvMatrix(2,:)*(angVel(:,1)-bg(:,1)-ng(:,1));gInvMatrix(3,:)*(angVel(:,1)-bg(:,1)-ng(:,1));rotMatrix(1,:)*(a(:,1)-ba(:,1)-na(:,1));
        %rotMatrix(2,:)*(acc(:,1)-ba(:,1)-na(:,1));-9.81+rotMatrix(3,:)*(acc(:,1)-ba(:,1)-na(:,1));nbgx;nbgy;nbgz;nbax;nbay;nbaz];
    xdot=[vx-nvx;vy-nvy;vz-nvz;gInvMatrix*(w-bg-ng);rotMatrix*(a-ba-na);nbgx;nbgy;nbgz;nbax;nbay;nbaz];
...
% using vectorization for code optimization
i=1:3;
...
F=eye(15,15)+dt*A;
% calculate covaraince estimated
